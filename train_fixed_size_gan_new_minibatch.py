...
checkpoint_path = 'fixed_size_GAN_new_minibatch/'
...
flattened_shape = 32*32*3
gen_images_flattened = tf.reshape(gen_images, [-1, flattened_shape], name='gen_flat')
image_input_flattened = tf.reshape(real_input, [-1, flattened_shape], name='input_flat')
...
with tf.name_scope('zero_shot_discriminator'):
    ...
    print('SHAPEEE', zs_model_image_repr_real.get_shape())
...
with tf.name_scope('minibatch_discriminator'):
    discriminator_minibatch_real = Minibatch_discriminator2(zs_model_image_repr_real, real_label, reuse=False,
                                                            num_kernels=50)
    feats_mb_real = discriminator_minibatch_real.minibatch_features
    real_minibatch_output = discriminator_minibatch_real.logits

    discriminator_minibatch_fake = Minibatch_discriminator2(zs_model_image_repr_fake, real_label, reuse=True,
                                                            num_kernels=50)
...
with tf.Session() as sess:
    ...
    for epoch in range(epochs):
        ...
        for batch_xs, batch_ys in train_generator:
            ...
            ng, ndr, ndf = sess.run([gen_loss_minibatch, disc_minibatch_loss_real, disc_minibatch_loss_fake], feed_dict={gen_input: noise, real_input: normalized_batch,
                                                                   real_label: batch_ys, fake_label: fake_label_batch})
            mean_gen_loss.append(ng)
            mean_disc_real_loss.append(ndr)
            mean_disc_fake_loss.append(ndf)

        print('G', np.mean(mean_gen_loss), 'DR', np.mean(mean_disc_real_loss),
              'DF', np.mean(mean_disc_fake_loss))
